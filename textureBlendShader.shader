Shader "Custom/textureBlendShader" 
{
    Properties
    {
        _MainTint("Diffuse Tint", Color) = (1,1,1,1)
   	  // Add the properties below so we can input all the textures
    ...
    SubShader
    {
	  ...
	  struct Input
	  {
        ...
        void surf (Input IN, inout SurfaceOutput o)
        {
   		...
   		// youneed a float 4 here because the texture
   		// will return R,G,B,and A or X,Y,Z, and W
   		float4 blendData = tex2D(_BlendTex, IN.uv_BlendTex);
   		// Get the data from the textures to blend
   		float4 rTexData = tex2D(_RTexture, IN.uv_RTexture);
   		float4 gTexData = tex2D(_GTexture, IN.uv_GTexture);
   		float4 bTexData = tex2D(_BTexture, IN.uv_BTexture);
   		float4 aTexData = tex2D(_ATexture, IN.uv_ATexture);
		//Construct a new RGBA value and add
		...
		finalColor.a = 1.0;
		// Add on the terrain tinting colors
